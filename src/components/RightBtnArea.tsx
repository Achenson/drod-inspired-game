...
function RightBtnArea({boardWidth, handleKeysOrBtns, setTextOnHover}: Props): JSX.Element {
  ...
  return (
    <div className="grid grid-cols-3 col-gap-4 row-gap-4" style={{ width: `${boardWidth/2}px` }}>
      {buttonsRight.map((el, i) => {
        return <TouchBtnRight key={i} btnType={el} handleKeysOrBtns={handleKeysOrBtns} setTextOnHover={setTextOnHover} />;
      })}
    </div>
  );
}
